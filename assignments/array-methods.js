...
runners.map(runner => {
    let upperCaseFirstName = runner.first_name.toUpperCase();
...
let total = ticketPrices.reduce((totalDonation, currentDonation) => {
    return totalDonation + currentDonation;
});
...
// Problem 1
// Oh no there has been a change in plans for the 5K and it has been delayed a week. Gather a list of emails in order to send out a mass email to all participants and update them.
let listOfEmails = [];

runners.filter(runner => {
    listOfEmails.push(runner.email);
});

console.log(listOfEmails);

// Problem 2
// You promised all of the top donating companies a special plaque at the end of the 5K please sort the find the top 3 companies that donated the most money
let topDonators = [];
let donations = [];

let donatorsToShow = 3;
...
topDonators.sort(function (a, b) {
    return b.donation - a.donation;
});

let reduceArrayAmount = topDonators.length - donatorsToShow;

topDonators.length -= reduceArrayAmount;

console.log(topDonators);


// Problem 3
// In order to make giving everyone runner their gear more efficient we need to sort their last names alphabetically.
let lastName = [];

runners.forEach(runner => {
    lastName.push(runner.last_name);
});
